...
- **자동 소거는 GC-001 에서 화면에 드러내지 않는다** (MIG-003 · 3.1.1). 판정은 `candidatesFrom()` 이
  그대로 하되, 표시는 `main.ts` 의 `SYSTEM_CLEARING` 하나가 정해 칠판·카드월·경찰서 좌석 세 곳에
  넘긴다. 생성 사건(400-seed 월드)은 기존대로 소거를 보여준다. **세 곳 중 하나만 고치면 어긋난다.**
- **`src/main.ts` 3,861줄 · `src/ui/crimescene3d.ts` 1,969줄** — 분해 대상. 다만 **테스트가 얇은 구간**이라
  분해 전에 특성 테스트부터 세워야 한다.
- **UI·3D 테스트는 아직 얇다.** 593건 중 대부분이 순수 엔진에 몰려 있다. 다만 드로어·사이드바·
...
**피드백 번호 규칙:** 새 피드백은 해당 화면의 마지막 번호 다음으로 이어 붙인다 (예: 취조실 마지막이 3.3.5면 다음은 3.3.6). `3.2.5.1` 식 하위 집은 만들지 않는다. Core Loop 이관분은 `MIG-xxx` 를 쓴다. 구번호↔신번호 대조는 `docs/피드백-번호-대조표.md`.
