/**
 * 골든 케이스 001 「옮겨진 상자의 사각」 — 생성기를 타지 않는 **수제 CaseFile**.
 *
 ...
 * 2. **자물쇠의 열쇠가 기록 2건이다** (E8 카메라 조각 + E6 반입대 작업 기록).
 *    정본 C-03("두 반쪽 연결")의 재현 — 상자 동선과 카메라 조각을 다 쥐어야
 *    라벨 대조가 열리고, 그래야 문소라의 대리 사용 가능성이 지워진다.
 *
 * ## 수단·신체·도구 묘사 금지 (정본 §4)
...
export function gc001Case(): CaseFile {
  ...
  return {
    ...
    /**
     * 기록 9건. 즉시 조회 7 + 해금 2 — 현장 예산 5회로 전부 열 수 없다 (선택이 곧 수사).
    ...
    evidence: [
      ...
      {
        // 카메라 조각 — 김하늘이 "시야가 열렸었다" 를 인정해야 그 구간을 뒤질 수 있다
      ...
      },
      {
         ...
         * 열쇠가 기록 2건인 이유(정본 C-03): 카메라 조각(E8)은 "라벨이 바뀌었다" 를,
         * 반입대 작업 기록(E6)은 "그 시각 문소라는 2분 거리에 있었다" 를 준다.
         * 둘을 다 쥐어야 대리 사용 가능성이 지워지고 일련번호 대조가 의미를 갖는다.
         */
    ...
    // 제시 반응 (정본 §12). 해금 쌍이 범인 전용이 아니다 — "열렸다 = 범인" 누설이 이 사건엔 없다.
    presentUnlocks: [
