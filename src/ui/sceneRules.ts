...
export const SCENE_FX = {
   ...
   * **1인칭 상한 속도 m/s.**
   *
   * 한때 2.4 로 눌렀다 — 키로 몸을 돌리던 시절엔 그게 맞았다. 조향이 느리니
   * 빠른 이동이 곧 미끄러짐으로 읽혔다. 마우스로 둘러보게 되자 정반대가 됐다:
   * 시선이 즉시 도니 몸만 굼떠 "너무 느리다" 가 됐다(사용자 실플레이).
   * 30초에 19×14m 홀을 훑어야 하므로 클립 속도(3.4)보다도 위인 4.2 로 올린다 —
   * 발이 미끄러져 보이는 대가를 치르더라도 시간 안에 닿는 것이 먼저다.
   */
  fpSpeed: 4.2,
  /**
...
/**
 * 시점별 이동 상한. 1인칭은 **fpSpeed 를 그대로 쓴다** — 클립 속도로 누르지 않는다.
 *
 * 예전엔 `min(clipSpeed, fpSpeed)` 로 클립보다 빠르지 않게 묶었다("발이 미끄러지면
 * 속도가 거짓말이 된다"). 그 규칙은 3인칭에서만 참이다 — 몸이 보이니까.
 * 1인칭은 몸이 화면에 없어서 발과 속도가 어긋나도 보이지 않고, 대신 **느린 것은
 * 그대로 보인다.** 사용자 실플레이가 "너무 느리다" 로 판정했으므로 상한을 푼다.
 */
export function moveSpeedFor(clipSpeed: number, firstPerson: boolean): number {
  return firstPerson ? SCENE_FX.fpSpeed : clipSpeed
}
 ...
 *
 * **`autopsy` 는 한 번 비었다가 돌아왔다.** 첫 왕진 가방 에셋(Sketchfab
 * "Doctor's bag")에는 톱과 붉은 액체 약병이 함께 모델링돼 있어 정본 §4
 * (수단·신체·유혈 묘사 금지)에 걸렸다. 카메라 각도로는 못 가렸고(톱이 가방 뒤에
 * 선 별개 덩어리라 어느 방위각에서도 위로 나온다) 머티리얼 병합본이라 부분 삭제도
 * 삼각형 단위 수술이 됐다. 그래서 잠시 증거 깃발로 대체했다가,
 * **금칙에 걸리지 않는 다른 가방("The Medical Bag")으로 교체해 되살렸다.**
 */
