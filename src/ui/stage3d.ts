...
export async function mount(host: HTMLElement, slug: string): Promise<Stage3D | null> {
  ...
  try {
    ...
    room.traverse((o) => {
      ...
      if (m.isMesh) {
        ...
        if (mm?.isMeshStandardMaterial) {
           ...
           * 휘도를 유지한 채 회색으로 **68%** 끌어당기면 나무의 결(명암)은 남고 갈색기가 빠진다.
           * (이전 55%에서는 아직 따뜻한 톤이 보였다 — 68%로 올려 먹색·회색 중심으로 읽히게.)
           * 사람에게는 이 처리를 하지 않는다 — 피부에서 채도를 빼면 시신처럼 보인다.
           */
          const l = mm.color.r * 0.2126 + mm.color.g * 0.7152 + mm.color.b * 0.0722
          mm.color.lerp(new THREE.Color(l, l, l), 0.68)
          mm.color.multiplyScalar(0.62)
    ...
    /**
     * ═══════════════════════════════════════════════════════════════════
     * 조명 상수 — 팀 3-3-(1) "취조실 색감을 먹색·회색 중심으로"
     * ═══════════════════════════════════════════════════════════════════
     *
     * 방 에셋의 갓등 **아래에** 실제 광원을 매단다.
     * 에셋에는 조명이 없다(LIGHT 0개). 등은 형상일 뿐이라 빛은 우리가 넣어야 한다.
     *
     * 원칙: 얼굴을 어둡게 하지 않는다 — 주변을 내려서 대비를 만든다.
     *        밝기를 낮추는 게 아니라 **범위를 줄인다.**
     *
     * ┌──────────────────────┬────────────┬───────────────────────────┐
     * │ 상수                 │ 값         │ 근거                      │
     * ├──────────────────────┼────────────┼───────────────────────────┤
     * │ KEY_COLOR            │ 0xfff4e8   │ 따뜻하되 갈색 아닌 텅스텐 │
     * │ KEY_INTENSITY        │ 17         │ 얼굴 밝기 유지            │
     * │ KEY_DISTANCE         │ 6.5m       │ 벽까지 닿지만 약해짐      │
     * │ KEY_ANGLE            │ π/5.8      │ 좁은 원뿔 = 얼굴+테이블   │
     * │ KEY_PENUMBRA         │ 0.70       │ 경계 부드럽게             │
     * │ KEY_DECAY            │ 1.6        │ 거리 감쇠                 │
     * │ HEMI_SKY             │ 0x262d35   │ 먹색·회색 하늘            │
     * │ HEMI_GROUND          │ 0x07090a   │ 거의 검정                 │
     * │ HEMI_INTENSITY       │ 0.18       │ 벽 형태만 읽힐 정도       │
     * │ FILL_COLOR           │ 0x7d96b8   │ 차가운 푸른기 (대비)      │
     * │ FILL_INTENSITY       │ 0.5        │ 키 대비 약 1/34           │
     * │ ROOM_DESAT           │ 0.68       │ 회색으로 끌어당기는 비율   │
     * │ ROOM_DIM             │ 0.62       │ 재질 배율                 │
     * │ ENV_MAP_INTENSITY    │ 0.25       │ 반사 억제                 │
     * └──────────────────────┴────────────┴───────────────────────────┘
     *
     * 이력:
     *   키 색: 0xffe2bd(갈색 과다) → 0xfff2e2(2221f8d) → 0xfff4e8(현재, 더 중성)
     *   키 콘: π/4.6 → π/5.4(2221f8d) → π/5.8(현재, 인물 집중 강화)
     *   환경광: 0x243040/0.16 → 0x2b333c/0.2(2221f8d) → 0x262d35/0.18(현재)
     *   재질 탈색: 0.55(2221f8d) → 0.68(현재)
     *   재질 배율: 0.66(2221f8d) → 0.62(현재)
     */
    const key = new THREE.SpotLight(0xfff4e8, 17, 6.5, Math.PI / 5.8, 0.70, 1.6)
    key.castShadow = true
    ...
    scene.add(new THREE.HemisphereLight(0x262d35, 0x07090a, 0.18))
    /**
     * 차가운 필 — **따뜻한 키와 대비를 만든다.** 단일 광원만 있으면 밝은 곳과
     * 검은 곳뿐이라 재질이 안 읽힌다. 푸른 필이 그늘에 정보를 남기고,
     * 그 색 대비가 "단색으로 눌림" 을 푼다. 키 대비 약 1/34.
     */
    const fill = new THREE.DirectionalLight(0x7d96b8, 0.5)
    ...
    const tick = (): void => {
      ...
      // 기본(heat=0): R=1.0 G=0.957 B=0.910 ≈ 0xfff4e8 (중성 텅스텐)
      // 최대(heat=1): R=1.0 G=0.737 B=0.590 (압박 붉은빛)
      key.color.setRGB(1, 0.957 - heat * 0.22, 0.910 - heat * 0.32)
      key.intensity = (17 + heat * 4) * flick
