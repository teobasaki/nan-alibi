 ...
 *
 * ⚠️ 이 파일의 화면 설명 문자열은 **작은따옴표 안에 작은따옴표를 넣지 마라.**
 *   문자열이 끊겨 생성되는 임시 테스트가 통째로 깨지고, 원인이 "vitest 실패" 로만 보인다.
 *   이 함정을 두 번 밟았다. 인용이 필요하면 큰따옴표(\") 를 쓴다.
 *
 * 사용법:
...
it('t', () => {
  ...
  for (let i = 0; i < ${n}; i++) {
    ...
    out.push({
      ...
      note: [
        '[기록 목록] 범행 시각 기록과 그 밖의 기록이 이름 붙은 두 구역으로 나뉘어 표시되고(위 구역은 범행 시각 기록 = 후보를 지우는 기록, 아래 구역은 그 밖의 시각 = 해금·교차검증용), 각 기록에 기록번호가 붙어 같은 시각·장소 기록도 구별된다. 각 기록에 용도가 함께 적힌다 — 범행 시각 기록은 "후보 소거", 그 밖은 "해금·교차검증". 사람을 지우는 건 범행 시각 기록뿐이며 조회 전에 이 구분이 보인다.',
        ...
        '[상단] 사건 개요와 범행 추정 시각, 남은 조사 표시(●), 그리고 상태에 맞춘 코치 한 줄. 조사를 3회 이상 쓰고도 범행 시각 기록을 하나도 열지 않았으면 그 사실을 경고한다. 남은 조사가 2회 이하가 되면 남은 후보 수와 우선순위를 함께 알려준다(후보가 여럿이고 범행 시각 기록이 남아 있으면 "승패는 범인 적중입니다 — 먼저 22:20 기록을 열어 후보를 줄이십시오. 제시는 완주 보상 경로입니다"). 무엇을 포기하는 중인지도 알려준다(예: "남은 조사 2회. 현장 기록은 아직 잠겨 있습니다 — 지금 결론을 내면 결정적 증거(20점)는 포기하는 것입니다.").',
        ...
        '[소리] 인장이 찍힐 때·자물쇠가 움직일 때·무반응일 때·기록을 꺼낼 때·결과가 날 때 각각 다른 확인음이 난다. 상단바에 음소거 토글이 있다.',
        '[기록] 통산 판수·해결 건수·최고점이 브리핑과 결과 화면에 표시된다.',
        '[결과 화면] 범인을 맞혔더라도 기록으로 후보가 좁혀졌는지를 구분해 적는다 — 한 명까지 좁혔으면 "추리가 완성된 판", 여럿 남았으면 "좁혀서 맞힌 것이 아니라 남은 후보 중에서 고른 것" 이라고 명시한다. 진범·동기·수단, 범인의 실제 이동 경로 재구성, 점수 내역. 결정적 증거를 틀렸으면 무엇이 정답이었는지와 왜 그것인지를 복기해 준다.',
      ].join(' '),
    })
  }
...
- **배점: 범인은 좁힌 만큼 — 후보 1명까지 좁혔으면 60, 2~3명이면 50, 4명 이상이면 40(적중 시)
  · 수단 20(결정적 증거 카드를 쥔 경우에만) · 결정적 증거 20 · 남은 조사 ×5 · 발견한 모순 ×5.**
...
/**
 * ⚠️ **한 번 0건은 수렴이 아니다.**
 * 리뷰어는 LLM 이라 같은 빌드에서 12판을 돌려도 0건과 5건이 번갈아 나온다.
 * 실제로 0건을 보고 "수렴" 이라고 선언한 직후 같은 빌드가 5건을 냈다.
 * 판정은 **연속 CLEAN_STREAK 회 0건** 일 때만 내린다.
 */
const STREAK_PATH = 'docs/.review-streak'
const need = Number(process.env.CLEAN_STREAK ?? 3)
let streak = existsSync(STREAK_PATH) ? Number(readFileSync(STREAK_PATH, 'utf-8').trim()) || 0 : 0
streak = all.length === 0 ? streak + 1 : 0
writeFileSync(STREAK_PATH, String(streak))

if (all.length > 0) {
  console.log(`❌ 미수렴 — 수정 후 재실행 (연속 무지적 0/${need})`)
  process.exit(1)
}
if (streak < need) {
  console.log(`◻︎ 이번 판은 지적 없음 — 연속 ${streak}/${need}. 리뷰어는 결정적이지 않으니 더 돌린다.`)
  process.exit(1)
}
console.log(`✅ 수렴 — 연속 ${streak}회 무지적`)
process.exit(0)
