  ...
  for (let i = 0; i < ${n}; i++) {
    ...
    out.push({
      ...
      note: [
        ...
        '[상단] 사건 개요와 범행 추정 시각, 남은 조사 표시(●), 그리고 상태에 맞춘 코치 한 줄. 남은 조사가 2회 이하가 되면 남은 후보 수와 우선순위를 함께 알려준다(후보가 여럿이고 범행 시각 기록이 남아 있으면 "승패는 범인 적중입니다 — 먼저 22:20 기록을 열어 후보를 줄이십시오. 제시는 완주 보상 경로입니다"). 무엇을 포기하는 중인지도 알려준다(예: "남은 조사 2회. 현장 기록은 아직 잠겨 있습니다 — 지금 결론을 내면 결정적 증거(20점)는 포기하는 것입니다.").',
