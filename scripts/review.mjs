...
it('t', () => {
  ...
  for (let i = 0; i < ${n}; i++) {
    ...
    out.push({
      ...
      note: [
        '[화면 구성] 왼쪽=용의자 5인(이름·직업·관계·압박 정도). 가운데=알리바이 대조표(위에 그린 것) + 그 아래 발견한 모순 목록. 오른쪽=조회 가능한 기록 목록, 잠긴 기록, 확보한 기록.',
        ...
        '[증거 제시 결과] 무반응이면 "이 사람은 이 기록에 대해 열어줄 진술이 없다 — 해금 경로에서 소거됐다", 반응이면 "잠긴 현장 기록 조건이 1/2 → 2/2 로 진전됐다" 또는 "자물쇠가 풀렸다" 로 진행도를 알려준다. 무반응 조합은 화면에 남는다.',
        '[상단] 사건 개요와 범행 추정 시각, 남은 조사 표시(●), 그리고 상태에 맞춘 코치 한 줄(예: "기록 카드와 진술 카드를 하나씩 눌러 연결하십시오. 무료입니다.").',
        '[시작 화면] 사건 브리핑이 한 번 뜨고 할 일 5단계를 안내한다: 대조표를 읽는다 / 심문하거나 기록을 조회한다 / 기록과 진술을 맞춰 본다 / 인장이 찍힌 사람에게 그 기록을 들이민다 / 범인·수단·결정적 증거를 지목한다.',
        '[제출 화면] 지금까지의 근거를 한 줄로 요약해 준다(조사 N회 소모 · 확보한 기록 N건 · 찾아낸 인장 N건 · 소거한 조합 N건). 결정적 증거는 원칙만 안내하고(범인이 범행 시각에 현장에 있었음을 확정하는 기록) 정답 표시는 하지 않는다. 범행 시각이 아닌 기록은 선택 불가로 회색 처리된다 — 범행 시각 기록은 여러 건이므로 어느 장소가 현장인가는 플레이어가 판단한다.',
        '[범행 수단] 결정적 증거는 언제나 범행 시각 현장의 카드키 출입 기록이고, 그 카드에는 "발급 구분"(예: 복제 의심 / 마스터 / 여벌 / 분실 신고분 / 임시)이 찍혀 있다. 그것이 곧 범행 수단이다 — 결정적 증거를 확보한 플레이어는 수단을 판독할 수 있고, 못 얻었으면 추측이다. 제출 화면이 이 규칙을 안내한다.',
